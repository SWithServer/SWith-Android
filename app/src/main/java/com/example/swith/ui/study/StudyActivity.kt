            ...
            with(supportFragmentManager) {
                when (item.itemId) {
                    R.id.bottom_nav_round -> {
                        ...
                        beginTransaction().apply {
                            if (prevRoundFragment.contains("RoundTabFragment"))
                                ...
                                prevRoundFragment =
                                    if (fragments[0] is RoundTabFragment || (fragments.size > 2 && fragments[1] is RoundTabFragment)) {
                                        replace(R.id.study_frm, RoundTabFragment())
                                            .commitAllowingStateLoss()
                                        "RoundTabFragment"
                                    } else {
                    ...
                    R.id.bottom_nav_calendar -> {
                        if (fragments[0] is RoundTabFragment || fragments[0] is RoundFragment) prevRoundFragment =
                            fragments[0].toString()
                        else if (fragments.size > 1 && (fragments[1] is RoundTabFragment || fragments[1] is RoundFragment)) prevRoundFragment =
