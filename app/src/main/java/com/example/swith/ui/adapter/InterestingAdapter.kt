...
class InterestingAdapter(private val interestingClickCallback: InterestingCallback) : RecyclerView.Adapter<InterestingViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): InterestingViewHolder {
        val binding = DataBindingUtil.inflate<ItemInterestingBinding>(LayoutInflater.from(parent.context), R.layout.item_interesting, parent, false)
    ...
    override fun onBindViewHolder(holder: InterestingViewHolder, position: Int) {
        dataList?.let {
            ...
            holder.binding.isSelect = (it[position] == interesting1) ||(it[position] == interesting2)
...
class InterestingViewHolder(val binding: ItemInterestingBinding) : RecyclerView.ViewHolder(binding.root)
