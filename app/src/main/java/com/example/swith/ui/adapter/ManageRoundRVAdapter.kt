...
class ManageRoundRVAdapter : RecyclerView.Adapter<ManageRoundRVAdapter.ViewHolder>(),
    ...
    inner class ViewHolder(val binding: ItemMangeRoundBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(round: GetSessionRes) {
            with(binding) {
                tvItemManageRoundCount.text = "${round.sessionNum}회차"
                tvItemManageRoundDate.text =
                    "${round.sessionStart[0]}/${round.sessionStart[1]}/${round.sessionStart[2]}"
            }
        }
    }
